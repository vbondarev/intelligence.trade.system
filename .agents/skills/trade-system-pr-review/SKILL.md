...
11. Проверь соблюдение языковой политики: человекочитаемый текст, XML documentation, комментарии и review-пояснения должны быть на русском языке; общеупотребимые технические термины и идентификаторы не переводи. Сверяй изменённый текст с исходным смыслом и контрактами: перевод не должен удалять идентификаторы, искажать wire values, условия, инварианты или смысл комментария.
