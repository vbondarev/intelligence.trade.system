...
- Связанный Issue является source of truth для scope, требований и критериев приёмки конкретной задачи. Если новое указание пользователя меняет scope, требования или критерии приёмки Issue, останови реализацию. До продолжения Issue должен быть синхронизирован с новым решением либо конфликт должен быть явно разрешён человеком. Не продолжай работу только на основании описанного расхождения.
- Для нетривиальной задачи сначала подготовь implementation plan без изменения кода. План должен перечислять затрагиваемые компоненты, архитектурные последствия, тесты, документацию, риски и предполагаемые границы изменения. Не начинай реализацию до завершения implementation plan. Если план выявил неоднозначность, архитектурный выбор, конфликт требований или необходимость выйти за scope Issue, остановись и передай вопрос человеку. В остальных случаях после завершения плана переходи к реализации самостоятельно.
