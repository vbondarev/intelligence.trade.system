...
- Для нетривиальной задачи сначала подготовь implementation plan без изменения кода. План должен перечислять затрагиваемые компоненты, архитектурные последствия, тесты, документацию, риски и предполагаемые границы изменения. Не переходи к реализации до явного принятия плана человеком.
