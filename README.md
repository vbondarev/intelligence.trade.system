...
Этапы A и B завершены. Проект умеет анализировать публичный рынок и содержит чистый бизнес-домен сопровождения аккаунта, позиции, портфеля, оценки и рекомендации. Полноценный пользовательский сценарий ещё не реализован: доменные объекты пока не сохраняются в PostgreSQL и не связаны с авторизацией, подключением аккаунта и периодической синхронизацией.
...
1. PostgreSQL, миграции, репозитории и оптимистическая конкурентность;
2. авторизация, изоляция пользователей и безопасное хранение ключей;
3. подключение Bybit-аккаунта и периодическая синхронизация;
4. детерминированная оценка позиции и политика рекомендаций;
5. пользовательский REST API и SignalR;
6. React-панель;
7. непрерывное наблюдение и Telegram-уведомления;
