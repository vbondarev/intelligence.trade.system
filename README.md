...
- инфраструктура структурированного логирования, OpenTelemetry и устойчивости внешних вызовов; полная телеметрия пользовательского пути появится вместе с синхронизацией и рекомендациями.

Биржевые компоненты ещё не связаны с пользователем, постоянным хранилищем и периодической синхронизацией.
...
Этапы B и C завершены. Текущий активный этап — **D: подключение аккаунта Bybit только для чтения и синхронизация**.

Основная ближайшая последовательность:

1. подключение Bybit-аккаунта и периодическая синхронизация;
