...
Этапы **A–E завершены**. Этап **F продолжается**, F-01 — F-06 завершены в `develop`. F-07 реализован в feature branch/PR #137 и станет завершённым этапом только после merge в `develop`; после merge текущим следующим шагом будет F-08.

Этап F намеренно разбит на последовательные небольшие изменения: F-01 зафиксировал стабильные v1-контракты и стратегию миграции pre-v1 `api/exchange-accounts`; F-02 завершил канонический lifecycle биржевого аккаунта (`/api/v1/exchange-accounts`) и удалил pre-v1 маршруты; F-03 добавил позиции и account-scoped portfolio; F-04 добавил position-scoped market/candles; F-05 добавил evaluation workflow и read model; F-06 добавил position timeline с cursor pagination и type filtering; F-07 реализован в PR #137, а F-08 будет следующим после merge. При этом OpenAPI/API tests обновляются в каждом PR, который добавляет или меняет публичный контракт. React/BFF начинается только после завершения этой backend-границы.
