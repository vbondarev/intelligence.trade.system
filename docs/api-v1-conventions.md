...
смещения. Nullable-значения сохраняют возможность отсутствия и
сериализуются как `null`, если поле является частью контракта. Контроллеры v1
используют общие `V1JsonSerializerOptions`; serializer legacy
