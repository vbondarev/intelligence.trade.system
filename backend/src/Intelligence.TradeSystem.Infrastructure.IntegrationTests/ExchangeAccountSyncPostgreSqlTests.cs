...
public sealed class ExchangeAccountSyncPostgreSqlTests(PostgreSqlFixture fixture)
{
    ...
    public async Task Concurrent_syncs_discovering_different_new_positions_do_not_deadlock()
    {
        ...
        // всегда сообщает обо всех ещё открытых позициях и обо всём открытом за это время.
        // Благодаря superset-наблюдению итоговое состояние детерминировано: сохраняются обе позиции
