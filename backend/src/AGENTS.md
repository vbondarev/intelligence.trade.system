...
XML documentation и обычные комментарии в C# пишутся на русском языке. Технические термины, имена API, типов и членов кода сохраняются на языке оригинала, когда это соответствует общей языковой политике репозитория. Не добавляй комментарии без пояснительной ценности.
