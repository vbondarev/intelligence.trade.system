...
public sealed class PublicMarketDataCollectorTests
{
    [Theory]
    [InlineData(null)]
    [InlineData("")]
    ...
    [Fact]
    public void Constructor_Does_Not_Require_Private_Account_Provider()
    {
        var collector = CreateCollector();

        collector.Should().NotBeNull();
        typeof(CollectedPublicMarketData).GetProperty("WalletBalance").Should().BeNull();
        typeof(CollectedPublicMarketData).GetProperty("OpenPositions").Should().BeNull();
    }

    [Fact]
    public async Task CollectAsync_Collects_All_Expected_Public_Data_For_Derivatives_Market()
    {
        ...
        marketDataProvider.Setup(x => x.GetTickerAsync("BTCUSDT", MarketCategory.Linear, It.IsAny<CancellationToken>())).ReturnsAsync(ticker);
        marketDataProvider.Setup(x => x.GetOrderBookAsync("BTCUSDT", MarketCategory.Linear, 50, It.IsAny<CancellationToken>())).ReturnsAsync(orderBook);
        marketDataProvider.Setup(x => x.GetRecentTradesAsync("BTCUSDT", MarketCategory.Linear, 60, It.IsAny<CancellationToken>())).ReturnsAsync(trades);
        marketDataProvider.Setup(x => x.GetKlinesAsync("BTCUSDT", MarketCategory.Linear, KlineInterval.FifteenMinutes, null, null, 250, It.IsAny<CancellationToken>())).ReturnsAsync(m15Klines);
        marketDataProvider.Setup(x => x.GetKlinesAsync("BTCUSDT", MarketCategory.Linear, KlineInterval.OneHour, null, null, 250, It.IsAny<CancellationToken>())).ReturnsAsync(h1Klines);
        marketDataProvider.Setup(x => x.GetKlinesAsync("BTCUSDT", MarketCategory.Linear, KlineInterval.FourHours, null, null, 250, It.IsAny<CancellationToken>())).ReturnsAsync(h4Klines);
        marketDataProvider.Setup(x => x.GetKlinesAsync("BTCUSDT", MarketCategory.Linear, KlineInterval.OneDay, null, null, 250, It.IsAny<CancellationToken>())).ReturnsAsync(d1Klines);
        derivativesDataProvider.Setup(x => x.GetOpenInterestHistoryAsync("BTCUSDT", MarketCategory.Linear, OpenInterestInterval.FiveMinutes, null, null, 48, It.IsAny<CancellationToken>())).ReturnsAsync(openInterestEntries);
        derivativesDataProvider.Setup(x => x.GetFundingRateHistoryAsync("BTCUSDT", MarketCategory.Linear, null, null, 30, It.IsAny<CancellationToken>())).ReturnsAsync(fundingRateEntries);
        derivativesDataProvider.Setup(x => x.GetLongShortRatioHistoryAsync("BTCUSDT", MarketCategory.Linear, LongShortRatioPeriod.FiveMinutes, null, null, 50, It.IsAny<CancellationToken>())).ReturnsAsync(longShortRatioEntries);

        var collector = new PublicMarketDataCollector(marketDataProvider.Object, derivativesDataProvider.Object);
        ...
        derivativesDataProvider.VerifyAll();
    }
    ...
    public async Task CollectAsync_Skips_Derivatives_Only_Requests_For_Spot_Market()
    {
        ...
        marketDataProvider.Setup(x => x.GetOrderBookAsync("BTCUSDT", MarketCategory.Spot, 50, It.IsAny<CancellationToken>())).ReturnsAsync(CreateOrderBook(category: MarketCategory.Spot));
        marketDataProvider.Setup(x => x.GetRecentTradesAsync("BTCUSDT", MarketCategory.Spot, 60, It.IsAny<CancellationToken>())).ReturnsAsync(new[] { CreateTrade(category: MarketCategory.Spot) });
        marketDataProvider.Setup(x => x.GetKlinesAsync("BTCUSDT", MarketCategory.Spot, KlineInterval.FifteenMinutes, null, null, 250, It.IsAny<CancellationToken>())).ReturnsAsync(CreateKlines(KlineInterval.FifteenMinutes, 5, MarketCategory.Spot));
        marketDataProvider.Setup(x => x.GetKlinesAsync("BTCUSDT", MarketCategory.Spot, KlineInterval.OneHour, null, null, 250, It.IsAny<CancellationToken>())).ReturnsAsync(CreateKlines(KlineInterval.OneHour, 5, MarketCategory.Spot));
        marketDataProvider.Setup(x => x.GetKlinesAsync("BTCUSDT", MarketCategory.Spot, KlineInterval.FourHours, null, null, 250, It.IsAny<CancellationToken>())).ReturnsAsync(CreateKlines(KlineInterval.FourHours, 5, MarketCategory.Spot));
        marketDataProvider.Setup(x => x.GetKlinesAsync("BTCUSDT", MarketCategory.Spot, KlineInterval.OneDay, null, null, 250, It.IsAny<CancellationToken>())).ReturnsAsync(CreateKlines(KlineInterval.OneDay, 5, MarketCategory.Spot));
    ...
    private static PublicMarketDataCollector CreateCollector() =>
        new(new Mock<IMarketDataProvider>().Object, new Mock<IDerivativesDataProvider>().Object);
    ...
    private static OpenInterestEntry CreateOpenInterestEntry() => new("BTCUSDT", MarketCategory.Linear, DateTimeOffset.UtcNow, 1_000m);
    private static FundingRateEntry CreateFundingRateEntry() => new("BTCUSDT", MarketCategory.Linear, DateTimeOffset.UtcNow, 0.0002m);
    private static LongShortRatioEntry CreateLongShortRatioEntry() => new("BTCUSDT", MarketCategory.Linear, DateTimeOffset.UtcNow, 0.55m, 0.45m);
}
