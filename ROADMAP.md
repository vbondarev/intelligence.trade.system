...
- **Tech-G05** ✅ (Issue #150): production EF Core read path списка позиций получил opt-in PostgreSQL benchmark на отдельном `postgres:16-alpine` с 250 000 positions, 8 users и 24 accounts. Сопоставимый baseline на старом пути показал global continuation с одним запросом, Seq Scan/Sort и 18 540 удалённых фильтром строк для default deep cursor; финальный путь использует bounded per-account continuation: 4 round trips для 3 accounts, 153 bounded candidates и 23.75 ms repository median в измеренном прогоне. Public cursor semantics не изменены, ownership predicate сохраняется в каждом position query, а merge ограничен `accounts × (pageSize + 1)`. Финальный набор evidence-backed indexes: `ix_positions_list_order`, `ix_positions_list_account_order`, `ix_positions_list_closed_order` и SQL-managed `lower(instrument_id)` index. Отдельный 12-account probe зафиксировал 13 round trips, 612 candidates и 54.31 ms repository median. Benchmark не входит в обычный CI test loop и включается только через `ITS_RUN_POSITION_LIST_PERFORMANCE=1`.
