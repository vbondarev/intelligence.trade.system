...
19. Публичный `/api/market-analysis/*` и `llm-payload` 1.0 остаются отдельным legacy/BTC Daily Check сценарием. Новый React-клиент получает пользовательский рыночный контекст и свечи через position-scoped `/api/v1/*` endpoints и не зависит от legacy market-analysis API.
...
- ⬜ Полный пользовательский API v1 для управления биржевыми аккаунтами, account-scoped портфеля, позиций, рыночного контекста/свечей, evaluation и timeline.
...
- legacy `/api/market-analysis/*` сохраняется независимо и не становится контрактом React-клиента.
...
2. `positions/{id}/evaluation` объединяет текущий `PositionAssessment` и связанную текущую `Recommendation` в один согласованный пользовательский read model. `POST` явно запускает переоценку позиции, но не подменяет stale/partial private state автоматическим account sync.
...
4. `positions/{id}/market` возвращает актуальный пользовательский market context для карточки позиции; `positions/{id}/candles` возвращает свечи для графика. React не должен использовать legacy `/api/market-analysis/*` как основной UI contract.
5. `positions/{id}/timeline` объединяет пользовательскую историю существенных изменений позиции, оценок и рекомендаций и поддерживает cursor pagination и фильтры. Внутренние таблицы и агрегаты при этом остаются раздельными.
6. `PUT /exchange-accounts/{id}/credentials` проверяет новую read-only пару credentials и ротирует её без смены стабильного `ExchangeAccountId`.
7. SignalR отправляет только user-scoped сообщения об изменении/инвалидации состояния (`exchangeAccount.updated`, `portfolio.updated`, `position.updated`, `evaluation.updated` или эквивалентные версионированные контракты). Полное актуальное состояние клиент получает через REST.
8. Публичные операции `acknowledge` и `dismiss` не входят в F до определения их пользовательской семантики. В частности, `dismiss` нельзя публиковать как API-команду до решения, должно ли отклонение скрывать рекомендацию, приостанавливать её или действовать до существенного изменения состояния.

| Код | Задача | Статус | Критерий завершения |
|---|---|---|---|
| F-01 | Зафиксировать структуру `/api/v1` и стабильные пользовательские контракты | ⬜ | API организован по функциональным областям; DTO/read models не экспортируют доменные агрегаты напрямую; контроллеры не содержат торговых расчётов; несовместимые изменения получают новую версию |
| F-02 | Реализовать API жизненного цикла биржевого аккаунта | ⬜ | Пользователь может получить список подключений, подключить read-only аккаунт, повторно проверить credentials/permissions, безопасно ротировать credentials без смены `ExchangeAccountId`, запустить sync и отключить аккаунт |
| F-03 | Реализовать read API позиций и account-scoped портфеля | ⬜ | Доступны список/карточка позиции и `PortfolioState` конкретного exchange account; cross-user доступ скрыт; общий cross-account `/portfolio` не имитируется без соответствующей доменной модели |
| F-04 | Реализовать position-scoped market context и свечи | ⬜ | Страница позиции получает рыночные показатели и candle series через `/api/v1`, не завися от legacy `/api/market-analysis/*`; backend определяет exchange/symbol/category из user-scoped позиции |
| F-05 | Реализовать единый evaluation workflow и read model | ⬜ | `GET evaluation` возвращает согласованные assessment + current recommendation; `POST evaluation` явно запускает расчёт без неявного private sync и сохраняет safety semantics stale/partial/uncertain данных |
| F-06 | Реализовать timeline позиции, cursor pagination и фильтры | ⬜ | История позиции, evaluation/recommendation changes и существенные события доступны единым пользовательским timeline без загрузки всей истории |
| F-07 | Реализовать SignalR и user-scoped группы/события инвалидации | ⬜ | Пользователь не может подписаться на данные другого пользователя; после события или reconnect клиент может восстановить актуальное состояние через REST |
| F-08 | Добавить OpenAPI и контрактные тесты пользовательского API | ⬜ | OpenAPI описывает auth, ProblemDetails, pagination, enums и v1 endpoints; контракт автоматически проверяется и пригоден для последующей генерации типов/клиента React |

Результат этапа: backend предоставляет стабильный пользовательский API для управления read-only биржевыми подключениями, чтения позиции и account-scoped портфеля, получения рынка/свечей, явного evaluation и timeline; SignalR безопасно сообщает об изменениях, а REST остаётся источником актуального состояния.
...
| G-03 | Реализовать сводку account-scoped портфеля и список позиций | ⬜ | Видны PnL, риск, свежесть, состояние синхронизации, позиции под наблюдением и критические позиции выбранного подключения |
| G-04 | Реализовать страницу позиции | ⬜ | Видны параметры сделки, график, market context, ключевые уровни, evaluation, рекомендация, причины и условия пересмотра |
| G-05 | Реализовать timeline позиции | ⬜ | Видны увеличение, уменьшение и закрытие позиции, изменения оценки/решения и существенные рыночные события через единый постраничный timeline |
| G-06 | Подключить SignalR с восстановлением через REST | ⬜ | Realtime-события инвалидируют соответствующее клиентское состояние; после разрыва соединения клиент перечитывает актуальные REST resources |
...
|---:|---|---|
| 1 | Зафиксировать структуру API v1 и пользовательские контракты | F-01 |
...
| 8 | Зафиксировать OpenAPI и контрактные тесты v1 | F-08 |
...
5. Получить согласованный evaluation: детерминированную оценку и рекомендацию с понятными причинами и отдельным решением, допустимо ли увеличивать риск.
