...
Последний учтённый PR: C-05 — пересмотрена стратегия аутентификации универсального API
...
| C-05A | Реализовать основу OAuth/OIDC-аутентификации универсального API | ⬜ | Выбран и зафиксирован Authorization Server; настроен JWT Bearer resource server с issuer/audience/signing validation; stable `sub` связан с Domain `UserId`; добавлены integration tests; публичные market endpoints остаются anonymous |
...
| Код | Задача | Статус | Критерий завершения |
|---|---|---|---|
| G-01 | Создать каркас адаптивного приложения, BFF и вход пользователя | ⬜ | Работают React shell, BFF/session integration, OAuth/OIDC login flow, защищённые маршруты и восстановление browser session |
