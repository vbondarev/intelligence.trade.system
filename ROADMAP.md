...
Проверенная база: `develop` после merge PR #69 / Tech-01; Tech-02 выполняется отдельным PR
Последний учтённый PR: [#69 «#68: Укреплены контракты приватного биржевого адаптера и тесты»](https://github.com/vbondarev/intelligence.trade.system/pull/69)
...
- **Tech-02** 🚧 (отдельный текущий PR): единый `ProblemDetails` contract, центральный `IExceptionHandler` и безопасное mapping exception → HTTP.
- **Tech-03** ⬜ не входит в Tech-02 и остаётся отдельной задачей наблюдаемости.
