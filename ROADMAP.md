...
Текущий следующий этап: **E-04 — внешне конфигурируемая политика рекомендаций**
...
Статус этапа: 🟡 Частично реализован (PR E.1: E-01 — E-03, E-09, E-10).
...
| E-04 | Реализовать версионируемую `RecommendationPolicy` с внешним `PolicyDefinition` | ⬜ | Параметры политики загружаются из валидируемой внешней конфигурации; одинаковый вход + одна версия/hash политики дают одинаковый результат; архитектура допускает последующий переход к декларативным правилам и Rule Engine |
...
| E-09 | Запретить повышение риска при устаревших, неполных или неопределённых данных | ✅ | Неотключаемый safety-инвариант в коде выдаёт `DoNotAdd` при stale/partial/uncertain данных и не позволяет внешней конфигурации разрешить повышение риска или сформировать сильное действие без достаточных данных |
...
| 2 | Реализовать внешне конфигурируемую политику и жизненный цикл рекомендаций | E-04 — E-08 |
