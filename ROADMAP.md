...
Проверенная база: `develop` на коммите [`eee0f30`](https://github.com/vbondarev/intelligence.trade.system/commit/eee0f300dcab3a1f1a77f7b4847b14b6637989f4) перед реализацией D-04
Последний учтённый PR: #80 «Реализовал согласование состояния аккаунта Bybit с биржей» (#81)
...
Статус этапа: 🟡 Подключение, ручная синхронизация, D-03-согласование и D-04 идемпотентность реализованы; фоновая синхронизация остаётся впереди.
...
Результат этапа: пользователь подключает Bybit с правами только на чтение; состояние баланса и позиций сохраняется, восстанавливается после перезапуска и монотонно сходится при повторных или конкурентных sync.
...
| 2 | Обеспечить идемпотентность и защиту от ответов не по порядку | D-04 — D-06 |
