...
Проверенная база: `develop` на коммите [`a8517c2`](https://github.com/vbondarev/intelligence.trade.system/commit/a8517c226974a15535e22d561201c1190a16d1e1) перед реализацией D-05
Последний учтённый PR: #84 «Реализовал фоновую синхронизацию активных аккаунтов Bybit» (#84)
Текущий следующий этап: **D-06 — устойчивые события синхронизации**
...
- ✅ `IPrivateAccountProvider` и account-specific private adapter Bybit связаны с пользователем, хранилищем, ручной и фоновой синхронизацией.
...
- ✅ Периодическая синхронизация активных аккаунтов и позиций.
...
Статус этапа: ✅ Подключение, ручная и фоновая синхронизация, D-03-согласование и D-04 идемпотентность реализованы; устойчивые события остаются впереди.
...
Результат этапа: пользователь подключает Bybit с правами только на чтение; состояние баланса и позиций сохраняется, восстанавливается после перезапуска и монотонно сходится при ручных, фоновых и конкурентных sync.
...
|---:|---|---|
| 1 | Публиковать устойчивые события синхронизации | D-06 |
| 2 | Добавить общий кэш публичных снимков рынка | D-07 |
| 3 | Реализовать детерминированную оценку позиции | E-01 — E-03, E-09, E-10 |
| 4 | Реализовать политику и жизненный цикл рекомендаций | E-04 — E-08 |
| 5 | Добавить пользовательский REST API и SignalR | F-01 — F-06 |
| 6 | Создать адаптивную React-панель | G-01 — G-08 |
| 7 | Добавить фоновые циклы наблюдения | H-01 — H-06 |
| 8 | Добавить Telegram-уведомления и детерминированные объяснения | I-01 — I-07 |
...
| 2026-09-09 | 3.0 | Реализован D-04: добавлен persisted account observation watermark, bounded CAS retry без повторного Bybit IO, безопасные AlreadyApplied/Superseded outcomes, защита от старых position observations и PostgreSQL race tests; следующая задача — D-06, затем D-05 и D-07. |
