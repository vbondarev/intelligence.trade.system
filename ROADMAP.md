...
Проверенный коммит: `5bf3a7aa9e6a8d6e2b7c9f814c2e9647393fdbcf`  
Последний учтённый PR: [#28 — «Выделил MarketIntelligence и отделил рыночный анализ от портфеля»](https://github.com/vbondarev/intelligence.trade.system/pull/28)  
Дополнительный источник: [`ROADMAP.md` из ветки `task/31-add_roadmap`](https://github.com/vbondarev/intelligence.trade.system/blob/task/31-add_roadmap/ROADMAP.md), версия файла `87a4803`  
Текущий активный этап: **B — бизнес-домен аккаунта и позиции**  
Статус документа: **основная и единственная актуальная дорожная карта**; репозиторный `ROADMAP.md` использован как источник и больше не считается самостоятельным планом
...
6. OpenClaw получает данные только через backend API и не обращается напрямую к Bybit или базе данных.
7. Детерминированное ядро формирует оценку и действие; ИИ только объясняет готовое решение.
8. Схема `llm-payload` версии `1.0` сохраняется для действующей цепочки OpenClaw.
9. Автоматическое открытие, усреднение и закрытие сделок не входит в первый MVP.
...
|---|---|---|
| A | Архитектурный фундамент | 🟡 Основное завершено; остались технические задачи |
| B | Бизнес-домен аккаунта, позиции, оценки и рекомендации | 🚧 Текущий этап |
| C | Хранение, безопасность и пользователи | ⬜ Не начат |
...
| K | Расширенный ИИ-анализ | ⬜ Не начат |
...
- ✅ OpenClaw-цепочка технического анализа и публикации BTC-обзора в Telegram.
...
- ✅ Добавлены архитектурные, прикладные, API- и модульные тесты.
- ✅ CI для PR #28 успешно выполнил сборку, тесты и проверку Docker-образа API.
- ✅ Базовая обвязка OpenTelemetry и проверки состояния сервиса присутствует в `ServiceDefaults`.

### Есть только как заготовка

- 🟡 `OpenPosition`, `OpenPositionSnapshot`, `PortfolioSnapshot` и сборщик портфеля существуют, но не образуют полноценный домен сопровождения позиции.
- 🟡 `IPrivateAccountProvider` и приватные методы `BybitProvider` умеют читать баланс и позиции, но ещё не связаны с пользователем, хранилищем и синхронизацией.
- 🟡 Публичные и приватные возможности разделены интерфейсами, но реализованы одним `BybitProvider`; временный `IBybitProvider` всё ещё используется.
...
- ⬜ Периодическая синхронизация и история изменений позиций.
- ⬜ Доменные оценки позиции и рекомендации.
...
Статус этапа: 🟡 Основная архитектурная подготовка завершена; оставшиеся задачи являются техническим долгом и не блокируют начало этапа B.
...
| A-08 | Добавить автоматическую сквозную проверку OpenClaw-цепочки | ⬜ | Проверяется путь payload → технический отчёт → итоговый текст без публикации в Telegram |
| A-09 | Актуализировать README под новое видение продукта | ⬜ | README различает текущие возможности и целевой продукт |

Основной результат этапа уже достигнут в PR #28. Задачи A-05 — A-07 должны быть выполнены не позднее начала D-01, а A-08 и A-09 — отдельными небольшими PR или вместе с ближайшими связанными изменениями.

### Этап B. Создать бизнес-домен сопровождения позиций

Статус этапа: ⬜ Не начат; существующие модели считаются только заготовкой.
...
| B-04 | Реализовать переходы позиции | ⬜ | Проверены New, Updated, Increased, Reduced, Closed и stale/unknown |
...
Результат этапа: готов чистый домен без EF Core, ASP.NET Core, Bybit.Net и LLM SDK.
...
| C-02 | Подключить PostgreSQL и миграции | ⬜ | Чистая база разворачивается миграциями |
| C-03 | Сохранять аккаунты, позиции, версии, портфели, оценки и рекомендации | ⬜ | Состояние восстанавливается после перезапуска |
...
| C-08 | Добавить интеграционные тесты с PostgreSQL | ⬜ | Проверены миграции, конкурентность и изоляция пользователей |
...
| I-07 | Сохранить BTC Daily Check отдельным публичным сценарием | ✅ | Текущая OpenClaw-цепочка остаётся совместимой с `llm-payload` 1.0 |
...
### Этап K. Расширить ИИ-анализ

Статус этапа: 🔵 Вне первого MVP.

| Код | Задача | Статус | Критерий завершения |
|---|---|---|---|
| K-01 | Адаптировать технического агента к контексту позиции | ⬜ | Агент анализирует готовые данные позиции и рынка, не обращаясь к внутреннему состоянию напрямую |
| K-02 | Добавить агентов новостей, макроэкономики и социальных источников | ⬜ | Каждый источник имеет отдельный формализованный отчёт и оценку свежести |
| K-03 | При необходимости добавить on-chain анализ | ⬜ | Источник подключается расширением общего контракта |
| K-04 | Создать синтезатор позиции | ⬜ | Он обрабатывает конфликтующие сигналы и не обходит ограничения подсистемы риска backend |
| K-05 | Сохранять входы, версии моделей и результаты | ⬜ | Объяснение и стоимость запроса можно аудировать |
| K-06 | Ограничить стоимость и частоту ИИ-запросов | ⬜ | ИИ вызывается только для существенных событий или по явному запросу |
...
| L-08 | Удалить оставшиеся временные проекты и контракты | ⬜ | Нет `IBybitProvider`, общего `Abstractions` и неиспользуемых AI-контрактов |
...
|---:|---|---|
| 1 | Добавить домен аккаунта и жизненный цикл позиции | B-01 — B-05 |
| 2 | Добавить портфель, типы оценки и рекомендации | B-06 — B-09 |
| 3 | Добавить PostgreSQL, миграции и репозитории | C-01 — C-04, C-08 |
| 4 | Добавить авторизацию, изоляцию пользователей и защищённые ключи Bybit | C-05 — C-07 |
| 5 | Разделить адаптер Bybit и реализовать подключение аккаунта | A-05 — A-07, D-01 |
| 6 | Реализовать синхронизацию и историю позиций | D-02 — D-06 |
...
| 11 | Создать первую версию React-панели | G-01 — G-08 |
| 12 | Добавить непрерывное наблюдение и события | H-01 — H-06 |
| 13 | Добавить очередь уведомлений и Telegram-события позиций | I-01 — I-04 |
| 14 | Добавить объяснения рекомендаций | I-05, I-06 |
| 15 | Завершить минимальную наблюдаемость, устойчивость и эксплуатационные процедуры | L-01 — L-07 |
| 16 | Добавить сбор фактических результатов и метрики качества | J-01 — J-07 |
| 17 | Завершить удаление временных компонентов после перевода всех потребителей | L-08 |

Задачи A-08 и A-09 можно выполнить отдельным небольшим PR либо включить в ближайшие связанные изменения. Расширенный ИИ-анализ K и расширение продукта M планируются после проверки первого MVP. Этап N не начинается до накопления статистики J.
...
8. Действующая схема `llm-payload` 1.0 и OpenClaw-цепочка не сломаны.
