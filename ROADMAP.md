...
Проверенный коммит: `b2e90dfd17044a2fe9d0663f1311676a84ead4c0`  
Последний учтённый PR: [#46 — «#45: Добавлены оценка позиции и жизненный цикл рекомендации»](https://github.com/vbondarev/intelligence.trade.system/pull/46)  
...
- ✅ Создан `Infrastructure` с базовым EF Core `DbContext`, PostgreSQL provider, конфигурацией connection string и интеграционным smoke-тестом Testcontainers; доменная схема пока не подключена.
...
- 🟡 Подготовлена инфраструктурная точка подключения PostgreSQL, но миграции и постоянное хранение доменного состояния ещё не реализованы.
...
- ⬜ Постоянное хранение истории позиций, оценок и рекомендаций.
...
| C-02 | Подключить PostgreSQL и миграции | 🟡 | Подготовлена точка подключения и design-time основа; доменные миграции появятся после persistence-модели |
...
|---:|---|---|
| 1 | Добавить PostgreSQL, миграции и репозитории | C-01 — C-04, C-08 |
